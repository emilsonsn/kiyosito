...
class FileManager
{
    ...
    */
	public function upload(){
		...
		if ($this->old) {
            \Log::info('Remove old file:', ['file' => $this->old]);
            $this->removeFile();
	    }
    ...
    */
    ...
    protected function uploadFile()
    {
        ...
        $isVideo = (strtolower($this->file->getClientOriginalExtension()) === 'webm');
    
        // Use the appropriate directory based on file type
        $baseDir = $isVideo ? 'videos' : 'images';
        $this->path = str_replace(['images', 'videos'], $baseDir, $this->path);
...
// 	}
    ...
    */
    ...
    //     \Log::info('Removing file:', ['path' => $path]);
    ...
    //             @unlink($thumbPath);
    //             \Log::info('Thumbnail removed:', ['path' => $thumbPath]);
    //         }
    //     }
    
    //     // Remove video files from the video directory (if applicable)
    //     if (pathinfo($this->old, PATHINFO_EXTENSION) === 'webm') {
    //         $videoPath = str_replace('images', 'videos', $this->path) . '/' . $this->old;
    //         if (file_exists($videoPath) && is_file($videoPath)) {
    //             @unlink($videoPath);
    //             \Log::info('Video file removed:', ['path' => $videoPath]);
    //         }
    //     }
    // }
    ...
    {
        if (!$path) {
            // Determine if the old file is an image or video
            $oldExtension = strtolower(pathinfo($this->old, PATHINFO_EXTENSION));
            $isOldImage = in_array($oldExtension, ['jpg', 'jpeg', 'png', 'gif']);
    
            // Determine the correct directory for the old file
            $oldDir = $isOldImage ? 'images' : 'videos';
            $path = str_replace(['images', 'videos'], $oldDir, $this->path) . '/' . $this->old;
        }
        ...
        // Remove the thumbnail (if it exists)
        if ($this->thumb) {
            $thumbPath = dirname($path) . '/thumb_' . $this->old;
            if (file_exists($thumbPath) && is_file($thumbPath)) {
                @unlink($thumbPath);
                \Log::info('Thumbnail removed:', ['path' => $thumbPath]);
            }
