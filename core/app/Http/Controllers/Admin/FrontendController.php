...
class FrontendController extends Controller
{
        ...
        foreach ($request->except('_token', 'video') as $inputField => $val) {
            if ($inputField == 'has_image' && $imgJson) {
                foreach ($imgJson as $imgValKey => $imgJsonVal) {
                    $validationRule['image_input.'.$imgValKey] = ['nullable','file',new FileTypeValidate(['jpg','jpeg','png','gif','webm'])];
    ...
    protected function storeImage($imgJson,$type,$key,$image,$imgKey,$oldImage = null)
    {
        ...
        // Handle WEBM as video
        if ($image->getClientOriginalExtension() === 'webm') {
            $path = 'assets/videos/frontend/' . $key; // Optional: Store videos separately
        }
