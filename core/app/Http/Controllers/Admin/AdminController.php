...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // ICO Related
        $phase = Phase::query();
        $ico['total_phases']           = $phase->count();
        $ico['phase']                  = $phase->whereDate('start_date', '<=', Carbon::now())->whereDate('end_date', '>=', Carbon::now())->first() ?? '0';
        $ico['total_coin_transaction'] = CoinHistory::sum('amount');
        $ico['upcoming_phases']        = Phase::upcoming()->count() ;
        $ico['running_auctions']       = Auction::where('status',Status::AUCTION_RUNNING)->count();
        ...
        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart','deposit','withdrawals','ico'));
    }
