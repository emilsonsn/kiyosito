...
// function frontendImage($sectionName, $image, $size = null, $seo = false, $thumb = false)
// {
//     if ($seo) {
//         return getImage('assets/images/frontend/' . $sectionName . '/seo/' . $image, $size);
//     }
//     if ($thumb) {
//         return getImage('assets/images/frontend/' . $sectionName . '/thumb_' . $image, $size);
//     }
...
function frontendImage($sectionName, $image, $size = null, $seo = false, $thumb = false)
{
    ...
    $isVideo = $image && pathinfo($image, PATHINFO_EXTENSION) === 'webm';

    // Determine the base path based on file type
    $basePath = $isVideo ? 'assets/videos/frontend/' : 'assets/images/frontend/';

    // Construct the full path
    if ($seo) {
        $path = $basePath . $sectionName . '/seo/' . $image;
    } elseif ($thumb) {
        $path = $basePath . $sectionName . '/thumb_' . $image;
    } else {
        $path = $basePath . $sectionName . '/' . $image;
    }
...
function levelCommission($user, $amount)
{
    $i       = 1;
    $general = gs();
    $level   = Referral::count();
    while ($user != "" || $user != "0" || $i < $level) {
        $bonusSender   = User::find($user);
        $refer = User::find($bonusSender->ref_by);
        if ($refer == "") {
            break;
        }
        ...
        $transaction->details      = 'Level ' . ordinal($i) . ' referral commission from ' . @$bonusSender->username;
        $transaction->trx          = $trx;
        ...
        notify($refer, 'REFERRAL_COMMISSION', [
            ...
            'trx'          => $trx,
            'level'        =>  ordinal($i) . ' Referral Commission',
...
{
    return  $phase->sold / $phase->total_coin * 100;
}

function whitePaperLink()
{
    return  route('home') . '/' . getFilePath('whitePaper') . '/whitepaper.pdf?get=' . rand(100, 999);
}
